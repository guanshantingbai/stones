...
from trainer2 import initial_train, online_train
...
def main(args):
    ...
    model_supporting = models.create(args.net, dim=args.dim, pretrained=True)
    ...
    model_supporting = torch.nn.DataParallel(model_supporting).cuda()
    ...
    if args.freeze_BN is True:
        ...
        model_supporting.apply(set_bn_eval)

    new_param_ids_fc_layer = set(
        map(id, model.module.fc_layer.parameters()))
    new2_param_ids_fc_layer = set(
        map(id, model_supporting.module.fc_layer.parameters()))

    new_param_ids = new_param_ids_fc_layer
    new2_param_ids = new2_param_ids_fc_layer
    ...
    new2_params_fc = [p for p in model_supporting.module.parameters() if id(
        p) in new2_param_ids_fc_layer]
    base2_params = [p for p in model_supporting.module.parameters() if id(
        p) not in new2_param_ids]

    param_groups = [
        {'params': base_params, 'lr_mult': 0.1},
        {'params': new_params_fc, 'lr_mult': 1.0},
        {'params': base2_params, 'lr_mult': 0.1},
        {'params': new2_params_fc, 'lr_mult': 1.0}]
    ...
    model_list = [model, model_teacher, model_supporting]
